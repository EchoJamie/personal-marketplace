...
description: "Use when running Maven tests or checking test results in Java projects. CRITICAL: Always use bash ~/.claude/plugins/marketplaces/personal-marketplace/scripts/test.sh instead of mvnd test or mvn test commands via Bash. Automatically filters debug output and highlights failures. Triggered by run tests, test class X, check if tests pass, verify tests, all tests, test results, failing tests, or Maven test execution. Alternatives: mvnd test, mvn test, or running Maven commands directly."
---
